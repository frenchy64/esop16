...
In this way \coretyped{} is effectively a linter.
...
but since type checking is a function call away, it is easily integrated as editor
...
which Typed Clojure define bivariantly---with write and read type parameters
like the atomic reference \clj{(Atom2 Int Int)} that can write and read \clj{Int}.
Variable-arity polymorphism~\cite{variable-arity-cite-todo} enables us to 
assign a type to functions like \clj{swap!} (figure~\ref{main:fig:swap!}), which takes a mutable \emph{atom},
...
Gradual typing enforces invariants at languages boundaries via contracts.
...
CircleCI provides continuous integration services built with a mixture of open-
and closed-source Clojure.
Typed Clojure has been used at CircleCI in production Clojure systems for at least two years.

CirlceCI provided the first author access to the main closed-source backend system written in Clojure
and Typed Clojure.
We conducted a study of the effectiveness of Typed Clojure in helping
There is no clear metric for quantifying typed Clojure code, since untyped code
can be freely mixed and some seemingly typed namespaces are not actually checked
...
featuring them in some way, they were particularly useful in example~\ref{example:circleci} for instance.
