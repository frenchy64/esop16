...
instrumenting the Typed Clojure type checker to record how often
...
Of 11 typed namespaces of FIXME lines of code,
 there are 32 Java interactions in user code. \marginpar{(FIXME what is user code?)}
...
No typed multimethods are defined. \marginpar{FIXME: how many are
  called, and what kind of arguments?}
...
the default choice of \coretyped{} to disallow \clj{nil} as a method argument
is justified.
...
only 407 (22\%) of 1834 \clj{def} expressions are checked.
That means 1427 (78\%) are unchecked, either by an explicit \clj{:no-check} annotation
or \clj{tc-ignore} macro to suppress type checking,
