...
Also, programs cannot depend on  Typed Clojure's typs, meaning that
...
and closed-source.
Typed Clojure has been used at CircleCI in production systems for two years \cite{CircleCIUsesTC},
...
%
%We determined that
They maintain a mixed typed-untyped Clojure code base of
382 namespaces comprising around 55,000 lines, 
...
\paragraph{Lessons}
Based on our empirical survey, it's clear that HMaps and Java interoperability support
are vital features---they are used on average more than once per typed
function. 

Multimethods are less common
in our case studies. The entire CircleCI code base contained only 26 multimethods,
are contained in the actual 
