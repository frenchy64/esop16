...
Values include booleans, \nil{}, class literals, keywords (written with a : prefix like \mintinline{clojure}{:a}), 
...
For example, in checking \mintinline{clojure}{(do (throw (Exception.)) (inc :a))}
by T-Error we assume \orprop{\botprop{}}{\botprop{}} in checking \mintinline{clojure}{(inc :a)},
which is a contradiction by the logic system (L-False, figure~\ref{appendix:figure:proofsystem})
which allows us to assume \mintinline{clojure}{(inc :a)} is unreachable (and it clearly is by BE-Do1).

This works similarly with our motivating example. Intuitively, when evaluating
\mintinline{clojure}{(inc x)} we know \mintinline{clojure}{(assert (number? x))} has evaluated to a value,
so we assume \mintinline{clojure}{x} is a number.
...
so T-Local checks \mintinline{clojure}{x} as a \Number.
