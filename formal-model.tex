...
An elegant extension we introduce is simple exceptional control flow
with \doliteral{} expressions which can follow control flow like

\begin{minted}[]{clojure}
  (do (if (number? x) (throw (new Exception)) nil)
      (inc x))
\end{minted}

which resembles the macroexpansion of Clojure's precondition syntax
