...
and class instances. 
We write the class literal \clj{Boolean} as \Boolean{} and \clj{Keyword} as \Keyword{}.
The type \Value{\Keyword} is inhabited by \clj{Keyword} and \clj{:a} is of type \Keyword{}.
...
$$
By T-Local we know \clj{x} is a \clj{N}.
$$
...
Subtyping for HMaps (figure~\ref{main:figure:hmapsem})
...
We have already mentioned paths \path{\pathelem{}}{\x{}}
where \pathelem{} is a sequence of selectors on \x{} called \emph{path elements},
but we have not yet defined what components of a path can be.
...
demonstrating how the first conditional test is checked in example~\ref{example:decleaf}.
First we unfold the destructuring syntax.
