...
%TODO EXPLAIN what is a proposition?
...
\emph{Type Propositions} \prop{} make assertions like ``variable \clj{x} is of type \clj{Number}'' or
``variable \clj{x} is not \clj{nil}''---in our logical system we write these as
{\isprop{\NumberFull}{\x{}}}
and {\notprop{\Nil{}}{\x{}}}. 
...
We cover L-Update in more detail in section~\ref{sec:formalpaths}.
...
The propositions of the test \thenprop{\prop{1}} and \elseprop{\prop{1}}
...
by \fieldtypeliteral{} (figure~\ref{main:figure:javatyping}).
...
The other rules T-MethodStatic and T-NewStatic work similarly (figure~\ref{main:figure:javatyping}), varying
in the choice of nilability in the conversion function---method returns
are nilable and constructor returns are non-nilable.

The operational semantics B-Field, B-New and B-Method (figure~\ref{main:figure:javatyping}) simply evaluate their
...
Recalling the examples in section~\ref{sec:multioverview},
\isaliteral{} is
a subclassing test for classes, otherwise an equality test---we do not model
the semantics for vectors.

The main interest for the typing rule is \isacompareliteral{} 
...
To demonstrate,
\mintinline{clojure}{(isa? (class x) Keyword)}
