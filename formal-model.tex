...
Introducing a lexically scoped variable $x$ is a matter of
...
obtained via $x$ is of type \Number{}. A \emph{logical system}
must now \emph{prove}
a variable's type.
$$
\begin{array}{lr}
...
says under logical assumptions {\propenv{}}, variable \x{} is of type \t{}.
...
\begin{itemize}
  \item variable \makelocal{m} is represented as \makelocal{m}; 
  \item $\appexpone{\ccclass{\appexp{\makekw{lunch}}{\makelocal{m}}}}$ is represented as ${\path{\classpe{}}{\path{\keype{\makekw{lunch}}}{\makelocal{m}}}}$;
...
\emptyobject{} is the
\emph{empty object}. Non-empty objects \path{\pathelem{}}{\x{}} consist of a root variable \x{} and a \emph{path} \pathelem{},
a possibly-empty sequence of \emph{path elements} applied right-to-left starting from the root variable.
We use two path elements---{\classpe{}} and {\keype{k}}---representing the result
of a calling \classconst{} and the result of looking up a keyword $k$ respectively.
...
\begin{equation}
\qquad {\isprop{\Order}{\makelocal{o}}}
\end{equation}
...
\begin{equation}
  \qquad {\isprop{\Value{\makekw{combo}}}{\path{\keype{\makekw{Meal}}}{\makelocal{o}}}}
\end{equation}
and pronounced ``the ${\makekw{Meal}}$ path of variable \makelocal{o} is of type
{\Value{\makekw{combo}}}'', a keyword singleton type.

\paragraph{Logical system in action} 
To attain the type of \makelocal{o}, 
T-LocalProve says we must solve for \t{} in
$
{ \inpropenv 
  {\propenv{}}
  {\isprop {\t{}} {\makelocal{o}}}}
...
\paragraph{Extended to propositional logic}
We extend our logical statements to \emph{propositional logic}.
...
The core type rules are
given as \figref{main:figure:othertypingrules}. We introduce
the interesting rules while demonstrating how to check
the complement number predicate
\begin{equation}
...
The lambda rule T-Abs introduces \isprop{\Top}{\makelocal{d}}
to check the body.
With \propenv{} = \isprop{\Top}{\makelocal{d}},
T-If then checks the test {\appexp{\numberhuh{}}{\makelocal{d}}}
with T-App with three main steps.

First, the operator \numberhuh{} is checked with T-Const, which
uses 
the \constanttypeliteral{} relation to give types to constants.
...
Resuming {\appexp{\numberhuh{}}{\makelocal{d}}},
the operand \makelocal{d} is checked with
T-Local as
$$
\judgementselfrewrite{\propenv{}}
                     {\makelocal{d}}
...
previously discussed about variables.

Finally, T-App substitutes the operand's object for the parameter
---here \makelocal{d} for \numberhuh{}'s \x{}---yielding
$$
...
To check {\ifexp{\appexp{\numberhuh{}}{\makelocal{d}}}{\false{}}{\true{}}},
T-If then
introduces \isprop{\Number{}}{\makelocal{d}}
in the `then' branch and \notprop{\Number{}}{\makelocal{d}}
in the `else' branch.
