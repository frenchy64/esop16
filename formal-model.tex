...
In standard type systems, the 
%use type environments to track lexical type information.
rules for variables use \emph{type environments}.
%associating $x$ with its type $\Number$ (a number) in the type environment,
%%Correspondingly, 
%and checking the type of a variable is a simple map lookup.
$$
\begin{array}{lr}
\infer [LC-Let]
...
\figref{main:figure:termsyntax} gives the syntax for objects---non-empty objects 
\path{\pathelem{}}{\x{}} consist of a root variable \x{} and a \emph{path} \pathelem{},
consisting of
...
T0-Local says we must solve for \t{} in
$
{ \inpropenv 
  {\propenv{}}
  {\isprop {\t{}} {\makelocal{o}}}}
...
\paragraph{Branching propositions} 
%Expressions in test position communicate propositions to each branch.
Each test expression \e{1} in
\ifexp{\e{1}}{\e{2}}{\e{3}} has a `then' and an `else' proposition to check
\e{2} and \e{3} respectively.
For example, in \ifexp{\makelocal{o}}{\e{2}}{\e{3}}
we know {\makelocal{o}} is true in \e{2} and false in \e{3}.
%that can be assumed if the expression evaluates to a
%true value and a false value respectively.
%
The `then' and `else' propositions 
for \makelocal{o}
are $\notprop{\falsy{}}{\makelocal{o}}$ and $\isprop{\falsy{}}{\makelocal{o}}$ respectively.
...
The branches are checked with T-False and T-True respectively,
%$$
