...
Path elements are {\classpe{}} and {\keype{k}}, representing a call to \classconst{} and
...
propositions---a `then' proposition that can be assumed if the expression evaluates to a
true value, and an `else' proposition for evaluating to a false value.

For example, the `then' proposition for the expression $o$ is $\notprop{\falsy{}}{o}$.
This says if the expression $o$ evaluates to a true value, then $o$ is neither
of type \False or \Nil. Naturally, the `else' proposition $\isprop{\falsy{}}{o}$
says $o$ is either \False or \Nil.

Together, both propositions are called a \emph{proposition set}.
...
\NumberFull{}  to \Number{}, and 
\StringFull{}  to \String{}.
The type \Value{\Keyword} is inhabited by the class literal \Keyword{} and \makekw{a} is of type \Keyword{}.
We abbreviate 
\EmptyUnion{}     to \Bot{}, 
{\ValueNil}       to \Nil{}, 
...
To prevent ambiguity between zero-argument methods and fields, field accesses are written \fieldexp{\fld{}}{\e{}},
and method calls are written $\methodexp{\mth{}}{\e{}}{\overrightarrow{es}}$.
%and \clj{(new class es*)} is $\newexp{\class{}}{\overrightarrow{es}}$.

\egref{example:getparent-direct-constructor}, copied here,
$$
...
$$
Since \t{} = \Keyword{}, we check the method body like so
$$
...
\paragraph{Semantics}
Multimethod definition semantics are straightforward, and are given 
...
\subsection{Multimethods and Java}
\subsection{Multimethods and HMaps}
