...
\egref{example:getparent-direct-constructor} \clj{(.getParent (new File "a/b"))}
translates to
...
Recalling the \secref{sec:multioverview} examples,
\isaliteral{} is
a subclassing test for classes, otherwise an equality test.
%---we do not model the semantics for vectors
%
The key component of the T-IsA rule (\figref{main:figure:mmsyntax})
...
$$
The result type is \Nil, and the return object is \path{\keype{\makekw{bocce}}}{\makelocal{b}}.
...
$$
The result type is \Top, and the return object is \path{\keype{\makekw{bocce}}}{\makelocal{u}}.
...
However, lookups on \makekw{bocce} on {\Unionsplice{\Breakfast \Lunch}} maps are unsupported.
...
$$
%(The metavariable \propisnotmeta{} ranges over \t{} and \nottype{\t{}} (without variables).)
  \caption{Proof system update rule (full rules \figref{appendix:figure:proofsystem})}
...
handled by the L-Update rule (\figref{main:figure:lupdate})---it says
...
To demonstrate,
\makekw{lunch} meals hit the first HMap case and
...
The third \keype{\k{}} case updates unknown entries to be mapped to \Nil{} or absent.
The fourth \keype{\k{}} case updates unknown entries to be \emph{present}
when they do no overlap with \Nil{}.
