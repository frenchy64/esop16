...
The \clj{hi} multimethod dispatches on its arguments via the identity function.
...
and \clj{:fr} have string values (in \clj{:mandatory}) and that 
every other key is absent (in \clj{:complete?}).

HMap's default to being partially specified. The HMap shorthand annotating
\clj{lunch}---below---forgets 
...
%\end{exmp}
Fully specified HMap's are used to
...
Partially specified HMap's with a common dispatch key
