...
\subsection{Typed Clojure Introduction}
...
The function may take strings, but providing \clj{nil} (Clojure's name for Java's \emph{null})
...
Providing \clj{repeat} with a nilable first argument is disallowed,
but the \clj{when} expression ensures it can only be passed an integer.
