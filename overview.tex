...
%
Clojure provides easy interoperation with
existing Java libraries,
with
Java
values being as any other Clojure value. However, this smooth
...
%
\clj{greet} that takes and returns strings.
...
\clj{File}
instance, inferred as a nullable string.
...
\end{exmp}
%
Typed Clojure can optionally integrate with the Clojure compiler to avoid expensive reflective 
calls, however if a specific constructor, method, or field cannot be found based on the
...
Typed Clojure conservatively asserts Java methods and
constructor arguments cannot be passed \clj{nil}---this restriction can be
adjusted per method. Method targets can never be \clj{nil}.
...
picks the \clj{String} method because \clj{(isa? String String)}
is true, and
\clj{(isa? String File)}
is not.
...
Keywords look themselves up in a map when called, or return \clj{nil}.
%
\begin{exmp}
\begin{minted}{clojure}
...
\end{minted}
The inferred type for \clj{breakfast} holds two pieces of information---the known entries--\clj{:en}
...
(defalias RawKeyPair ; extra keys disallowed
  (HMap :mandatory {:pub RawKey, :priv RawKey}, :complete? true))
...
(defn enc-keypair [{pk :priv :as kp}] ; original map is kp
  (assoc (dissoc kp :priv) :enc-priv (encrypt pk)))
\end{minted}
...
As \clj{EncKeyPair} is fully specified, we must remove extra keys like \clj{:priv}.
%
\begin{minted}{clojure}
(defn enc-keypair-bad [{pk :priv :as kp}]
  (assoc kp :enc-priv (encrypt pk))) ; Type error: :priv disallowed
\end{minted}
...
linked.
There are infinite ways of structuring and dispatching on data,
so we cannot merely add a set of special case rules for
...
(defmethod desserts :lunch [{d :desserts}] d) ; d is :desserts entry
...
\begin{minted}{clojure}
(ann desserts' [Order -> Int])
(defmulti desserts' (fn [o :- Order] (class (:desserts o))))
(defmethod desserts' Long [{d :desserts :as o}] 
  ; o :- (U '{:Meal ':lunch, :desserts Int} '{:Meal ':dinner, :desserts Int}
