...
to allow \clj{nil}.
All Clojure values are true except \clj{nil} and \clj{false}, so a \clj{nil} test
amounts to a test on the value.
...
returning a nullable string \clj{"a"}.
...
resulting in 
\begin{minted}{clojure}
(.getName (new File "STAINS/JELLY")) ;=> "JELLY" :- (U nil Str)
\end{minted}
...
(defn enc-keypair-bad [{pk :priv :as kp}]
  (assoc kp :enc-priv (encrypt pk))) ; Type error: :pub disallowed
\end{minted}
The extra \clj{:priv} entry does not match \clj{EncKeyPair}, so a type error
is raised.
