...
paper we treat it as pointer equality.
...
data. The syntax \clj{^File x} is a single expression that is the symbol \clj{x} 
with metadata \clj{{:tag File}}.
In binding positions like \clj{(fn [^File x] ...)}
the metadata is preserved in occurrences of \clj{x}.
...
\clj{:as} and a symbol, which binds the symbol to the current argument.
$$
\clj{(fn [{x :x, y :y, :as m}] ...)}
$$ 
binds \clj{m} to the first argument,
\clj{x} to \clj{(get m :x)} and
\clj{y} to \clj{(get m :y)} in the function body. Type hints are preserved as expected.
...
The \clj{when} macro is a one-armed \clj{if} that returns \clj{nil} if the test fails.
The \clj{throw} special form is exactly Java's \java{throw}, which takes an
instance of \clj{Throwable} and throws an exception with it.
...
The \clj{defalias} macro defines a type abbreviation which can be recursively defined.
