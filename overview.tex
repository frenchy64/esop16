...
(defn greetings [n i]
  (str "Hello, " (when i  ; when i is a non-nil integer
...
(defn greetings-bad [n i]
  (str "Hello, " (apply str 
                   (repeat i ; Type Error: Expected Int, given (U nil Int).
                           "hello, "))
       n "!"))
\end{minted}
...
(:fr lunch)    ; :- Str
;=> "baguette"
(:bocce lunch) ; :- Any     ; Unknown lookups are now less accurate
;=> nil
\end{minted}
...
(defn enc-keypair-bad [{pk :priv :as kp}]
  (assoc kp :enc-priv (encrypt pk)))
  ; Type Error: Expected EncKeyPair, given 
  ;   (HMap :mandatory {:enc-priv EncKey, :priv RawKey, :pub RawKey}
  ;         :complete? true)
\end{minted}
...
\end{itemize}
Equivalently, we learn \clj{o} is of type
