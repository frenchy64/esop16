...
\end{lstlisting}
\caption{A simple Typed Clojure program (delimiter colors: {\color{interop}Java interoperation (green)}, 
  {\color{types}type annotation (blue)},
  {\color{invoke}function invocation (black)}, {\color{red}collection literal (red)}, {\color{mygray}miscellaneous (gray)})}
\label{fig:ex1}
\end{figure*}
